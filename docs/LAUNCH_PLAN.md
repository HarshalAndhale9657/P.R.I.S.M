...
- **W1 — Re-architecture skeleton:** ✅ *(2026-08-30; hardened 2026-09-06 — ADR-0019)* split into pluggable stages `parse → retrieve → match → rerank → ai_risk → triage → coach → report` behind interfaces; first-class `eval/` harness (run any stage over a dataset) + `models/` layer (download/cache/version); existing matcher/corpus become stage impls. Keep all 22 tests green.
- **W2 — Real benchmark = the gate (do first):** ✅ *(2026-08-30; in CI as per-dataset gates since 2026-09-06 — ADR-0020)* ingest **public paraphrase/similarity sets — PAWS, MRPC, STS-B, QQP (+ PAWS-X)**; run the ACTUAL pipeline; report P/R/**FPR per stratum** (incl. a high-overlap-but-not-paraphrase stratum ≈ ESL/boilerplate risk); **freeze as CI gate**, keep the 32-case synthetic set as a fast smoke. **No PAN.** This is the honest baseline.
- **W3 — Detection lift (bi-encoder, no training):** ✅ *measured 2026-08-30 — mpnet ≈ MiniLM, not shipped (honest negative result)* swap MiniLM → stronger ONNX embedder (`bge-base`/`gte-base`; multilingual for translated) behind `local_embeddings.embed()`; recalibrate thresholds on the real benchmark; **ship only if recall ↑ with FPR flat** (esp. high-overlap negatives); torch fallback until green.
- **W4 — Cross-encoder rerank + retrieval:** 🟡 *rerank stage shipped opt-in 2026-08-31; Semantic Scholar + OA full text still open (W4b)* pretrained cross-encoder over top-k candidates to catch **hard paraphrases** (current recall 0.77); add **Semantic Scholar** + **Unpaywall/arXiv/PMC OA full-text** (real full text, not just abstracts); re-eval, keep gate green.
- **W5 — Selective cross-encoder fine-tune (go/no-go):** 🟡 *kit ready 2026-08-31; needs one human-run GPU session* light fine-tune of the reranker on **PAWS/MRPC** (free Colab/Kaggle GPU, LoRA) → export to ONNX; **ship it only if it beats the pretrained cross-encoder on the W2 benchmark** without raising FPR — else keep pretrained and bank the week. *(AI detector is deferred behind this — ADR-0016.)*
- **W6 — Go live:** ✅ *packaging done 2026-09-06 (legacy deps removed, multi-stage image, Compose + Caddy, runbook in `deploy/`)*. Remaining: run the runbook on the **Hetzner CX32**; UptimeRobot on `/health/ready`; Sentry DSN; smoke a real 20-page PDF and **measure rerank latency there to decide the default**. (Frontend is served by Caddy on the same host — no Cloudflare Pages needed.)
...
reactivation around submission season · low refund/chargeback · **CI: public-dataset regression gates green (STS-B/MRPC/QQP at the confident cutoff — `eval/gates.json`; FPR=0.00 is not a meaningful target, ADR-0017)**.
