...
class AuthorshipClustering:
    ...
    def cluster(
        ...
        # IMPORTANT: Track which columns are semantic BEFORE filtering,
        # so we can apply the 0.20 weight to the correct columns AFTER filtering.
        feature_names = FEATURE_NAMES if len(FEATURE_NAMES) == feature_matrix.shape[1] else [f"feat_{i}" for i in range(feature_matrix.shape[1])]
        is_semantic = np.array([name.startswith("semantic_") for name in feature_names])

        valid_columns = column_variances >= 1e-10
        filtered_matrix = feature_matrix[:, valid_columns]
        # Carry the semantic mask through the same filter
        is_semantic_filtered = is_semantic[valid_columns]
        ...
        try:
            ...
            # Down-weight semantic features to prevent topic-based over-fragmentation.
            # Uses the boolean mask (tracks correct columns through filtering).
            if np.any(is_semantic_filtered):
                scaled_features[:, is_semantic_filtered] *= 0.20
